...
case "$1" in
    all_save)
        ...
        ;;
    window_save)
        scrot '%Y-%m-%d-%s_screenshot_$wx$h.jpg' -ue 'mv $f $$(xdg-user-dir PICTURES)'
        dunstify "Screenshot has been saved to `xdg-user-dir PICTURES`"
        ;;
    selection_save)
        dunstify "Selection will be saved to `xdg-user-dir PICTURES`"
        scrot '%Y-%m-%d-%s_screenshot_$wx$h.jpg' -se 'mv $f $$(xdg-user-dir PICTURES)'
        ;;
    selection_copy)
        dunstify 'Selection will be saved to /tmp/tmp_scrot'
        scrot -se 'mv $f /tmp/tmp_scrot && xclip -selection clipboard -t image/png -i /tmp/tmp_scrot'
        ;;
    *)
